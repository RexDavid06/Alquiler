...
MIDDLEWARE = [
    ...
    # Serves collected static files from the same process (single-container
    # free-tier hosting). No external web server required for the demo.
...
# CSRF trusted origins for the Django admin/docs on the deployed domain.
# DRF token auth does not enforce CSRF, but the browsable API and admin do.
...
# Web-server static backend (Whitenoise).
#
# Production uses the manifest backend for cache-busted assets. The manifest
# is generated by ``collectstatic`` and is NOT available in dev/tests (a
# missing manifest entry would otherwise break AdminPage tests and runserver),
# so DEBUG uses the plain compressed backend instead.
STORAGES = {
    'default': {
